...
RAGFlow owns DeepDoc as the default path, but `rag/app/naive.py` also routes datasets to other backends when the dataset config asks for them. The backend list currently includes `mineru` (`deepdoc/parser/mineru_parser.py`), `docling` (`deepdoc/parser/docling_parser.py`), `opendataloader`, `tcadp parser`, `paddleocr` (`deepdoc/parser/paddleocr_parser.py`), `somark`, and `plaintext`. `deepdoc` remains the default branch, and the other branches live in `deepdoc/parser/`. The app keeps the split simple: use DeepDoc when the default parser path fits, switch to another backend when the dataset needs a different extraction strategy, and then hand the result to the same merge and tokenization flow.
